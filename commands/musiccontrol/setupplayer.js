...
const server_remotemsg = new Map();
...
async function syncchannel(channel){
  ...
  await server_remotemsg.set(channel.guild.id, remote);
  setupplayer(channel);
...
async function setupplayer(channel){
  ...
  const adplayermsg = await server_remotemsg.get(channel.guild.id);

  const emoji = ["⏯️", "⏏️", "⏹️", "⏭️", "🔀", "🔂", "🔁", "♾️", "📶"];
  const additionalemoji = ["🔈", "🔊", "❌", "⤴️", "⤵️"];
  ...
  const collector = playermsg.createReactionCollector(filter, {}); let i = 0;
  ...
  collector.on("collect", async(reaction, user) => {
    ...
    if(reaction.emoji.name == emoji[1]){
      ...
      adplayermsg.reactions.removeAll();
      initplayer(channel);
    }
    if(!queue.isplaying && reaction.emoji.name != emoji[1]){
        ...
        switch (reaction.emoji.name){
          ...
          case emoji[8]:
            if(adplayermsg.reactions.cache.size > 0) break;
            for(let j = 0; j < additionalemoji.length; j++){
              await adplayermsg.react(additionalemoji[j]);
            }
            const adfilter = (reaction, user) => additionalemoji.includes(reaction.emoji.name); 
            const adcollector = adplayermsg.createReactionCollector(adfilter, {time: 60000});
            adcollector.on("collect", async(reaction, user) => {
              reaction.users.remove(user);
              if(!queue.isplaying){
                let warningmsg = await channel.send('노래가 현재 재생 중이 아니에요.');
              }else{
                switch(reaction.emoji.name){
                  case additionalemoji[0]:
                    queue.setVolume = (queue.setVolume - 0.1 <= 0) ? 0 : queue.setVolume - 0.1;
                    await queue.connection.dispatcher.setVolume(queue.setVolume);
                    await editnpplayer(channel);
                    break;

                  case additionalemoji[1]:
                    queue.setVolume = (queue.setVolume + 0.1 >= 1) ? 1 : queue.setVolume + 0.1;
                    await queue.connection.dispatcher.setVolume(queue.setVolume);
                    await editnpplayer(channel);
                    break;

                  case additionalemoji[2]:
                    if(queue.songs.length > 1){
                      await queue.songs.pop();
                    }
                    editnpplayer(channel);
                    break;

                  case additionalemoji[3]:
                    if(queue.songs.length > 1){
                      let tomove = queue.songs[queue.curq + 1];
                      await queue.songs.splice(queue.curq + 1, 1);
                      queue.songs.push(tomove);
                    }
                    editnpplayer(channel);
                    break;

                  case additionalemoji[4]:
                    if(queue.songs.length > 1){
                      let tomove2 = queue.songs[queue.songs.length - 1];
                      await queue.songs.splice(queue.songs.length - 1, 1);
                      await queue.songs.splice(1, 0, tomove2);
                    }
                    editnpplayer(channel);
                    break;
                }
            ...
            adcollector.on("end", () => adplayermsg.reactions.removeAll());
            break;
        }
      }
  });
...
async function editnpplayer(channel){
  ...
  if(!queue.connection){
    ...
    playstatus = queue.connection.dispatcher.paused ? '⏸️  일시정지됨' : '▶️  지금 재생 중!';
  }

  let embed = new Discord.MessageEmbed()
    .setColor('#FF6F61')
    .setDescription(`상태 : ${playstatus} | 루프: ${loopstatus} | 볼륨: ${Math.round(queue.setVolume * 100)}%`)
...
async function stopforbutton(channel, queue){
  await channel.guild.me.voice.channel.leave();
